...
# Hyperparameters
IMG_SIZE = (224, 224)
BATCH_SIZE = 32 # Aman untuk VRAM 12GB
EPOCHS_STAGE_1 = 30
EPOCHS_STAGE_2 = 50
...
def build_model():
    ...
    # Normalisasi khusus DenseNet (skala piksel)
...
callbacks_list = [
    ...
    callbacks.ModelCheckpoint(
        filepath='densenet121_best.keras', 
        monitor='val_loss', 
        save_best_only=True,
        verbose=1
    ),
...
history_stage_1 = model.fit(
    ...
    callbacks=callbacks_list + [callbacks.CSVLogger('densenet121_stage1.csv')] # Ubah ini
)
...
# Karena kita menggunakan BatchNormalization di dalam DenseNet, sangat disarankan 
# untuk membiarkan layer tersebut tetap beku agar statistik rata-rata/varians tidak rusak.
...
history_stage_2 = model.fit(
    ...
    class_weight=class_weight,
    callbacks=callbacks_list + [callbacks.CSVLogger('densenet121_stage2.csv')] # Ubah ini
)

print("\nPelatihan selesai! Bobot terbaik telah disimpan di 'densenet121_best.keras'.")